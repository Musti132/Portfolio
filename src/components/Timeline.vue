...
<template>
    <div class="timeline-container max-w-3xl mx-auto p-4 mt-6 relative pointer-events-none">

        <h1 class="text-4xl sm:text-6xl font-bold text-light mb-10 lg:text-left md:text-center break-words glow">Timeline</h1>
        <div class="timeline-line absolute transform mt-2 h-full border-l-1 -mx-3 border-secondary/20"></div>
        <div v-for="(yearBlock, index) in timelineEvents" :key="index" class="mb-12">
            <h2 class="text-3xl font-bold mb-6 text-light">{{ yearBlock.year }}</h2>
            <div v-for="(event, idx) in yearBlock.events" :key="idx" class="mt-8 pl-4 relative">
                <span
                    ...
                    :class="event.active ? 'bg-secondary animate-ping' : 'bg-gray-600'">
                </span>
                <span
                    v-if="event.active"
                    class="absolute -left-5 top-2.5 w-4 h-4 rounded-full flex items-center justify-center bg-secondary text-light font-semibold"></span>
                <h3 class="text-xl font-medium text-light">{{ event.title }}</h3>
                <p class="text-sm text-light/70 mb-2">{{ event.month }}</p>
                <p class="text-light/80">{{ event.description }}</p>
            </div>
        </div>
    </div>
</template>
